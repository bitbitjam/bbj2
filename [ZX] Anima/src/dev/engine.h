...
void mueve_bullets (void) {
	for (gpit = 0; gpit < MAX_BULLETS; gpit ++) {
		if (bullets [gpit].estado) {
			...
			if( flags [1] != 1 ) {
    			bullets [gpit].my ++;
	    		if(bullets [gpit].my > 4) bullets [gpit].estado = 0;
	        }
...
void mueve_bicharracos (void) {
	...
	for (gpit = 0; gpit < 3; gpit ++) {
		...
		if(n_pant == 10){
		    if (gpt == 1){
    		    gpt = malotes [30].t = 7;
                en_an [0].next_frame = sprite_21_a;
            } else if (gpt == 2){
    		    gpt = malotes [31].t = 7;
                en_an [1].next_frame = sprite_22_a;
            }
            
            active = 1;
            
			malotes [30].x += malotes [30].mx;
			malotes [30].y += malotes [30].my;
			malotes [31].x = malotes [30].x;
			malotes [31].y = malotes [30].y + 16;
			...
			if (gpen_cy > 120){
    			malotes [30].my = -malotes [30].my;
    	    }
			if (gpen_cx > 120){
    			malotes [30].mx = -malotes [30].mx;
    	    }
        }
...
#ifndef PLAYER_MOGGY_STYLE							
							if (malotes [enoffsmasi].t != 8) malotes [enoffsmasi].life --;
#else
